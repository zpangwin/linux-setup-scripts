...
if [[ "true" != "${reinstallActionsOnly}" ]]; then
	...
	echo "================================================================";
	sudo dnf install -q -y curl git ImageMagick perl unzip wget mono-complete mono-core;
...
if [[ "true" != "${omitActions}" ]]; then
	...
	if [[ 1 == $isSELinuxEnabled && 1 == $(which restorecon 2>/dev/null|wc -l) ]]; then
		echo "Restoring SELinux Filecontext on Nemoaction...";
		if [[ "true" == "${isUserInstall}" ]]; then
			find "${nemoActionsUserDir}" -iname '*gitext*.nemo_action*' -exec restorecon "{}" \;;

			restorecon "${userLauncherScriptsDir}";
			find "${userLauncherScriptsDir}" -iname '*gitext*.sh*' -exec restorecon "{}" \;;
		else
			sudo find "${nemoActionsSystemDir}" -iname '*gitext*.nemo_action*' -exec restorecon "{}" \;;

			sudo restorecon "${systemLauncherScriptsDir}";
			sudo find "${systemLauncherScriptsDir}" -iname '*gitext*.sh*' -exec restorecon "{}" \;;
		fi
