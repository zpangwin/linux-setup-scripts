...
isFunctionLoaded=$(declare -f addCustomSource|wc -l);
...
if [[ ! -e /etc/apt/sources.list.d/mono-official-stable.list ]]; then
	...
	if [[ "20.04" == "${ubuntuVersion}" ]]; then
		addCustomSource mono-official-stable "deb https://download.mono-project.com/repo/ubuntu stable-focal main";

	elif [[ "18.04" == "${ubuntuVersion}" ]]; then
		addCustomSource mono-official-stable "deb https://download.mono-project.com/repo/ubuntu stable-bionic main";
	fi
