...
# Verify variables
# ==================================================
if [[ "" == "${appDownloadLink}" ]]; then
	echo 'ERROR: appDownloadLink not defined.';
...
echo "Checking that gdebi is installed ...";
...
echo "Downloading antimicro DEB file ...";
cd "${tempDir}";
wget --user-agent "${userAgent}" "${appDownloadLink}" --output-document="${downloadPath}" >/dev/null;
if [[ "0" != "$?" ]]; then
...
installDependenciesFromDebFile "${downloadPath}" >/dev/null;

echo "Installing DEB file ...";
sudo dpkg -i "${downloadPath}" >/dev/null;
