...
WATERFOX_MENU_LINK="/usr/share/applications/waterfox.desktop";
PRIVATE_MENU_LINK="/usr/share/applications/waterfox-private-browsing.desktop";
PRIVATE_ICON_NAME="private-browsing";
PRIVATE_ICON_PATH="/usr/share/icons/${PRIVATE_ICON_NAME}.png";
WATERFOX_ICON_NAME="waterfox";
...
if [[ -e "${WATERFOX_INSTALL_DIR}/waterfox" ]]; then
	# Make sure system link exists
	if [[ ! -e /usr/bin/waterfox ]]; then
		sudo ln -s "${WATERFOX_INSTALL_DIR}/waterfox" /usr/bin/waterfox;
	fi
