...
if [[ "0" != "${isWaterfoxKpeInstalled}" ]]; then
    ...
    addAptCustomSource unofficial-waterfox-hawkeye116477 'deb http://download.opensuse.org/repositories/home:/hawkeye116477:/waterfox/xUbuntu_18.04/ /';
    ...
    if [[ -f /etc/apt/sources.list.d/unofficial-waterfox-hawkeye116477.list ]]; then
        ...
        sudo sed -i '1s/^/#\n\n/' /etc/apt/sources.list.d/unofficial-waterfox-hawkeye116477.list;
        sudo sed -i '1s|^|# See:\n# https://github.com/MrAlex94/Waterfox/commits?author=hawkeye116477\n|' /etc/apt/sources.list.d/unofficial-waterfox-hawkeye116477.list;
        sudo sed -i '1s/^/# his personal PPA personal rather than an official one.\n/' /etc/apt/sources.list.d/unofficial-waterfox-hawkeye116477.list;
        sudo sed -i '1s/^/# hawkeye116477 is a Waterfox developer. However, this is\n/' /etc/apt/sources.list.d/unofficial-waterfox-hawkeye116477.list;
    fi
    ...
    # update install flag
    isWaterfoxKpeInstalled=$(which waterfox-classic 2>/dev/null|wc -l);
fi
