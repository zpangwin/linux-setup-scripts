...
IS_APPLICATION_BIN_ON_PATH=$(which waterfox-classic 2>/dev/null|wc -l);
if [[ "0" == "${IS_APPLICATION_BIN_ON_PATH}" ]]; then
    APPLICATION_BIN_NAME="waterfox";
    IS_APPLICATION_BIN_ON_PATH=$(which waterfox 2>/dev/null|wc -l);
    if [[ "0" == "${IS_APPLICATION_BIN_ON_PATH}" ]]; then
...
if [[ ! -f "${PRIVATE_MENU_LINK}" ]]; then
    ...
    if [[ -f "${APPLICATION_MENU_LINK}" ]]; then
	    ...
	    sudo sed -i -E "s|^(Exec=\\S*) .*\$|\1 -new-instance -private -url ${PRIVATE_DEFAULT_PAGE}|g" "${PRIVATE_MENU_LINK}";
	else
		...
		echo "Exec=${APPLICATION_BIN_NAME} -new-instance -private -url ${PRIVATE_DEFAULT_PAGE}"|sudo tee -a "${PRIVATE_MENU_LINK}" 2>&1 >/dev/null;
